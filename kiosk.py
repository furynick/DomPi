...
# ✅ Initialisation de YTMusic avec OAuth
...
# ✅ Variables Partagées pour l'Interface Graphique
current_track_info = {
    'titre': 'Titre',
    'artiste': 'Artiste',
    'album': 'Album',
...
# ✅ Fonction : Sélection de la meilleure miniature
...
# ✅ Fonction pour télécharger et redimensionner la miniature
...
# ✅ Thread 1 : Traitement des playlists
def manage_playlist():
...
# ✅ Thread 2 : Extraction de l'URL audio
def manage_track():
    ydl_opts = {
        'quiet': True,
        'skip_download': True,
...
# ✅ Fonction : Lancement de FFmpeg (conversion_audio)
def conversion_audio(audio_url):
    try:
        cmd = [
            'ffmpeg',
            '-i', audio_url,
    ...
    while not stop_event.is_set():
        try:
            ...
            while retries > 0 and process is None:
                process = conversion_audio(audio_url)
                if process is None:
                    retries -= 1
                    time.sleep(2)

            if process is None:
                print(f"Échec de la conversion pour {audio_url}")
                q_play.task_done()
                continue
                ...
                if playing:
                    ...
                    try:
                        ...
                        if buffer_count == BUFFER_THRESHOLD and not buffer_ready_event.is_set():
                            print("Remplissage cible atteint")
                            buffer_ready_event.set()
                    except queue.Full:
                        print("File tampon de données audio pleine")
                        continue
...
def play_audio():
    ...
    while not stop_event.is_set():
        print("En attente que le tampon soit rempli pour le prochain morceau...")
        buffer_ready_event.wait()
        buffer_ready_event.clear()
        print("Tampon prêt. Début de la lecture audio.")

        while not stop_event.is_set():
            try:
                data = q_data.get(timeout=1)
                if data is None:
                    print("Fin du morceau détectée.")
                    q_data.task_done()
                    break
...
thread_gui = threading.Thread(target=buildMainUI)
